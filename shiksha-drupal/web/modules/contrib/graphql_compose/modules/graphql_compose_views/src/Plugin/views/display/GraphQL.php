...
class GraphQL extends DisplayPluginBase {
  ...
  public function getGraphQlResultType(): string {
    ...
    if ($this->getPlugin('row') instanceof GraphQLEntityRow) {
      $union_types = $this->getGraphQlUnionTypes();

      return match (count($union_types)) {
        0 => 'UnsupportedType',
        1 => reset($union_types),
        default => $this->getGraphQlRowName(),
      };
    }
  ...
  public function getGraphQlUnionTypes(): array {
    ...
    $view = $this->view;
    $manager = $this->gqlEntityTypeManager();

    // Use the content type union for base entity type.
    if ($view->getBaseEntityType()) {
      $entity_type_id = $view->getBaseEntityType()->id();
      $entity_type_plugin = $manager->getPluginInstance($entity_type_id);
      $types[] = $entity_type_plugin?->getUnionTypeSdl();
    }
