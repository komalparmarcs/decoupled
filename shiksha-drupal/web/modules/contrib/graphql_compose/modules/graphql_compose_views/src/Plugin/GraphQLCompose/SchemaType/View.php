...
class View extends GraphQLComposeSchemaTypeBase {
  ...
  public function getUnionTypes(GraphQL $display) {
    ...
    if (!$display->getPlugin('row') instanceof GraphQLEntityRow) {
      return [];
    }

    // Only need to create a new union type if there are multiple entity types.
    // getGraphQlResultType will use the first union type if there is only one.
    $union_types = $display->getGraphQlUnionTypes();
    if (count($union_types) <= 1) {
      return [];
    }
